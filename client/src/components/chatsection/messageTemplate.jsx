...
export const MessageTemplate = ({ chat, userId }) => {
  ...
  return (
    <div className={`flex ${containerClass} my-2 mx-3`}>
      <div
        className={`max-w-xs md:max-w-sm lg:max-w-md px-4 py-2 rounded-lg shadow-md ${messageClass}`}
      >
        <div className="break-words">
          {chat.content && <p>{chat.content}</p>}
          {chat.file && (
            // <a
            //   href={chat.file.url || '#'} // Adjust this to the actual file path
            //   target="_blank"
            //   rel="noopener noreferrer"
            //   className="text-blue-400 underline break-words"
            // >
            //   {chat.file.name || 'View File'}
            // </a>
            <img src={chat.file} alt="" />
          )}
        </div>
        <div className="text-xs text-gray-400 text-right mt-1">
          {moment(chat.createdAt).format("MMM DD, YYYY h:mm A")}
